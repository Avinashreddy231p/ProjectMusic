...
import com.mardous.booming.data.local.room.TemporalColumns
...
class RealTimeStatsTracker(
    ...
    private val pendingGroups = mutableListOf<com.mardous.booming.data.local.room.ListeningSessionGroupEntity>()
    ...
    fun onSongTransition(
        ...
        if (session != null) {
            ...
            sessionCoordinator.onSongPlayed(System.currentTimeMillis())
        }
    ...
    fun drainPendingGroups(): List<com.mardous.booming.data.local.room.ListeningSessionGroupEntity> {
        synchronized(lock) {
    ...
    fun endCurrentSessionGroup(): com.mardous.booming.data.local.room.ListeningSessionGroupEntity? {
        val summary = sessionCoordinator.onSessionEnd() ?: return null
        val group = com.mardous.booming.data.local.room.ListeningSessionGroupEntity(
    ...
    companion object {
        ...
        private fun buildSession(
            ...
            val endReasonFinal = when {
                endReason == "track_finished" && completionPercent < 90.0 -> "seek"
                else -> endReason
            }
