package com.mardous.projectmusic.core.sort
...
import com.mardous.projectmusic.core.model.sort.DescendingItem
import com.mardous.projectmusic.core.model.sort.KeySortItem
import com.mardous.projectmusic.core.model.sort.SortItem
...
import com.mardous.projectmusic.extensions.media.asReadableTrackNumber
import com.mardous.projectmusic.extensions.media.normalizeForSorting
import com.mardous.projectmusic.data.local.repository.StatsCache
...
sealed class MoodSortMode(
    id: String,
    defaults: Pair<SortKey, Boolean>,
    items: List<SortItem>
) : SortMode(id, defaults, items) {

    object AllMoods : MoodSortMode(
        id = "mood",
        defaults = SortKey.AZ to false,
        items = listOf(
            KeySortItem.Title,
    ...
    fun List<Mood>.sorted(): List<Mood> {
        val moods = when (selectedKey) {
            SortKey.AZ -> sortedWith(compareBy(collator) { it.name.normalize() })
            SortKey.SongCount -> sortedWith(compareBy { it.songCount })
            SortKey.PlayCount -> sortedWith(compareBy { it.playCount })
            SortKey.ListenDuration -> sortedWith(compareBy { it.listenedDurationMs })
            SortKey.Rank -> sortedWith(compareBy { it.overallRank })
            SortKey.DateAdded -> sortedWith(compareBy { it.createdAt })
            SortKey.DateModified -> sortedWith(compareBy { it.updatedAt })
            else -> this
        }
        return if (selectedDescending) moods.reversed() else moods
    }
}

sealed class TagSortMode(
    id: String,
    defaults: Pair<SortKey, Boolean>,
    items: List<SortItem>
) : SortMode(id, defaults, items) {

    object AllTags : TagSortMode(
        id = "tag",
        defaults = SortKey.AZ to false,
        items = listOf(
            KeySortItem.Title,
    ...
    fun List<Tag>.sorted(): List<Tag> {
        val tags = when (selectedKey) {
            SortKey.AZ -> sortedWith(compareBy(collator) { it.name.normalize() })
            SortKey.SongCount -> sortedWith(compareBy { it.songCount })
            SortKey.PlayCount -> sortedWith(compareBy { it.playCount })
            SortKey.ListenDuration -> sortedWith(compareBy { it.listenedDurationMs })
        ...
        return if (selectedDescending) tags.reversed() else tags
    }
}

sealed class InstrumentSortMode(
    id: String,
    defaults: Pair<SortKey, Boolean>,
    items: List<SortItem>
) : SortMode(id, defaults, items) {

    object AllInstruments : InstrumentSortMode(
        id = "instrument",
        defaults = SortKey.AZ to false,
        items = listOf(
            KeySortItem.Title,
    ...
    fun List<Instrument>.sorted(): List<Instrument> {
        val instruments = when (selectedKey) {
            SortKey.AZ -> sortedWith(compareBy(collator) { it.name.normalize() })
            SortKey.SongCount -> sortedWith(compareBy { it.songCount })
            SortKey.PlayCount -> sortedWith(compareBy { it.playCount })
            SortKey.ListenDuration -> sortedWith(compareBy { it.listenedDurationMs })
            SortKey.Rank -> sortedWith(compareBy { it.overallRank })
            SortKey.Confidence -> sortedWith(compareBy { it.avgConfidence })
            SortKey.DateAdded -> sortedWith(compareBy { it.createdAt })
            SortKey.DateModified -> sortedWith(compareBy { it.updatedAt })
            else -> this
        }
        return if (selectedDescending) instruments.reversed() else instruments
    }
}
