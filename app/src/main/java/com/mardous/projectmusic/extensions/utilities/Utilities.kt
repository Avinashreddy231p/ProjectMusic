...
package com.mardous.projectmusic.extensions.utilities
...
inline fun <reified T : Enum<T>> Int?.toEnum() =
    this?.let { ord ->
        enumValues<T>().firstOrNull { it.ordinal == ord }
    }
...
inline fun <reified T> String?.deserialize(defaultValue: T): T {
    ...
    return if (!isNullOrEmpty())
        runCatching<T> { lenientJson.decodeFromString(this) }
            .onFailure { Log.d("ProjectMusicUtilities", "Json.decodeFromString($this): error", it) }
