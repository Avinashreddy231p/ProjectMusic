...
import com.mardous.projectmusic.core.model.player.ProgressState
import com.mardous.projectmusic.data.model.Song
import com.mardous.projectmusic.extensions.getShapeAppearanceModel
...
import com.mardous.projectmusic.ui.screen.player.PlayerAnimator
import com.mardous.projectmusic.ui.screen.player.PlayerViewModel
import com.mardous.projectmusic.util.ANIMATE_PLAYER_CONTROL
import com.mardous.projectmusic.util.DISPLAY_ALBUM_TITLE
import com.mardous.projectmusic.util.DISPLAY_EXTRA_INFO
import com.mardous.projectmusic.util.ENABLE_SCROLLING_TEXT
import com.mardous.projectmusic.util.NOW_PLAYING_EXTRA_INFO
import com.mardous.projectmusic.util.PREFER_ALBUM_ARTIST_NAME
...
abstract class AbsPlayerControlsFragment(@LayoutRes layoutRes: Int) : Fragment(layoutRes),
    ...
    override fun onResume() {
        super.onResume()
        musicSlider?.setProgressBarStyle(Preferences.progressBarStyle)
        musicSlider?.setThumbStyle(Preferences.thumbStyle)
        musicSlider?.setProgressControlStyle(Preferences.progressControlStyle)
    ...
    open fun onUpdateShuffleMode(shuffleModeEnabled: Boolean) {
        val iconResource = if (shuffleModeEnabled) R.drawable.ic_shuffle_on_24dp else R.drawable.ic_shuffle_24dp
        shuffleButton?.let {
            it.setIconResource(iconResource)
            it.applyColor(
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String?) {
        when (key) {
            ...
            SQUIGGLY_SEEK_BAR, PROGRESS_BAR_STYLE, THUMB_STYLE, PROGRESS_CONTROL_STYLE -> {
                musicSlider?.setProgressBarStyle(Preferences.progressBarStyle)
                musicSlider?.setThumbStyle(Preferences.thumbStyle)
                musicSlider?.setProgressControlStyle(Preferences.progressControlStyle)
