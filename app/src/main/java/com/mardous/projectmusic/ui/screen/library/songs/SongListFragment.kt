...
package com.mardous.projectmusic.ui.screen.library.songs
...
import com.mardous.projectmusic.util.SongExportHelper
...
class SongListFragment : AbsRecyclerViewCustomGridSizeFragment<SongAdapter, GridLayoutManager>(), ISongCallback {
    ...
    override fun createAdapter(): SongAdapter {
        ...
        val adapter = SongAdapter(
            ...
            swipeContext = com.mardous.projectmusic.core.model.swipe.SwipeContext.SONGS,
            callback = this
        )
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        ...
        if (item.itemId == R.id.action_export_db) {
            lifecycleScope.launch {
                try {
                    val result = SongExportHelper(requireContext(), rankingDao).export()
                    val shareIntent = Intent(Intent.ACTION_SEND_MULTIPLE).apply {
                        type = "*/*"
                        putParcelableArrayListExtra(Intent.EXTRA_STREAM, arrayListOf(result.csvUri, result.jsonUri))
                        addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
                    }
                    startActivity(Intent.createChooser(shareIntent, getString(R.string.action_export_db)))
                } catch (e: Exception) {
                    android.widget.Toast.makeText(context, R.string.stats_export_failed, android.widget.Toast.LENGTH_SHORT).show()
                }
            }
            return true
        }
