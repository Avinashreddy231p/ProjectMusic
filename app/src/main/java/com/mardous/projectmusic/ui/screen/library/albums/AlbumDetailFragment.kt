...
package com.mardous.projectmusic.ui.screen.library.albums
...
import com.mardous.projectmusic.databinding.FragmentAlbumDetailBinding
...
class AlbumDetailFragment : AbsMainActivityFragment(R.layout.fragment_album_detail),
    ...
    private fun createSongAdapter() {
        ...
        simpleSongAdapter = SimpleSongAdapter(
            ...
            swipeContext = com.mardous.projectmusic.core.model.swipe.SwipeContext.ALBUMS,
            callback = this
        )
    }
    ...
    private fun setupRecyclerView() {
        headerAdapter = HeaderAdapter { view ->
            ...
            val rankInfo = buildList {
                if (album.overallRank > 0) add("Rank: #${album.overallRank}")
                if (album.playCount > 0) add("Played: ${album.playCount}x")
                if (album.listeningDuration > 0) add("Listened: ${album.listeningDuration.asReadableDuration(readableFormat = true)}")
            }.joinToString(" • ")
            ...
            if (Preferences.uiTheme == UITheme.SPOTIFY) {
                ...
                headerBinding.image.albumImage(album) { crossfade(false) }

                headerBinding.title.text = album.name
                ...
                headerBinding.playAction.setOnClickListener {
                    playerViewModel.openQueue(album.songs, shuffleMode = OpenShuffleMode.Off, queueSource = "album")
                }
                headerBinding.shuffleAction.setOnClickListener {
                    playerViewModel.openAndShuffleQueue(album.songs)
                }
            } else {
