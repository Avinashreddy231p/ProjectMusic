...
import com.mardous.projectmusic.databinding.ActivityTagEditorBinding
...
abstract class AbsTagEditorActivity : AbsBaseActivity(),
    ...
    protected open fun selectImage() {
        val items = arrayOf(
            getString(com.mardous.projectmusic.R.string.download_image),
            getString(com.mardous.projectmusic.R.string.pick_from_local_storage),
            getString(com.mardous.projectmusic.R.string.web_search),
            getString(com.mardous.projectmusic.R.string.restore_default),
            getString(com.mardous.projectmusic.R.string.remove_cover)
        )
        MaterialAlertDialogBuilder(this)
            .setTitle(com.mardous.projectmusic.R.string.update_image)
    ...
    private fun writeTags() {
        viewModel.write(this, propertyMap).observe(this) { result ->
            if (result.isLoading) {
                ...
                showToast(com.mardous.projectmusic.R.string.saving_changes)
            } else {
                binding.actionSave.show()
                if (result.isSuccess) {
                    showToast(com.mardous.projectmusic.R.string.changes_saved_successfully)
                } else {
                    showToast(com.mardous.projectmusic.R.string.could_not_save_some_changes)
                }
            }
        }
    }
