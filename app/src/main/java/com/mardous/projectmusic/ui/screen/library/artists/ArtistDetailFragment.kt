...
import com.mardous.projectmusic.core.sort.AlbumSortMode
import com.mardous.projectmusic.core.sort.SongSortMode
import com.mardous.projectmusic.core.sort.SortMode
...
import com.mardous.projectmusic.data.remote.lastfm.model.LastFmArtist
import com.mardous.projectmusic.databinding.FragmentArtistDetailBinding
import com.mardous.projectmusic.extensions.applyHorizontalWindowInsets
import com.mardous.projectmusic.extensions.defaultGridColumns
...
import com.mardous.projectmusic.extensions.navigation.playInfoArgs
import com.mardous.projectmusic.extensions.navigation.searchArgs
import com.mardous.projectmusic.extensions.plurals
import com.mardous.projectmusic.extensions.resources.removeHorizontalMarginIfRequired
import com.mardous.projectmusic.extensions.resources.setupStatusBarForeground
import com.mardous.projectmusic.extensions.resources.surfaceColor
...
import com.mardous.projectmusic.ui.adapters.HeaderAdapter
import com.mardous.projectmusic.ui.adapters.HorizontalListAdapter
import com.mardous.projectmusic.ui.adapters.SectionHeaderAdapter
import com.mardous.projectmusic.ui.adapters.WikiAdapter
import com.mardous.projectmusic.ui.adapters.album.SimpleAlbumAdapter
import com.mardous.projectmusic.ui.adapters.artist.ArtistAdapter
import com.mardous.projectmusic.ui.adapters.song.SimpleSongAdapter
...
import com.mardous.projectmusic.databinding.ItemDetailHeaderBinding
import com.mardous.projectmusic.databinding.ItemSpotifyDetailHeaderBinding
...
class ArtistDetailFragment : AbsMainActivityFragment(R.layout.fragment_artist_detail),
    ...
    override fun onCreateView(
        ...
        val isSpotify = Preferences.uiTheme == com.mardous.projectmusic.util.UITheme.SPOTIFY
        return if (isSpotify) {
    ...
    private fun createSongAdapter() {
        ...
        songAdapter = SimpleSongAdapter(
            ...
            swipeContext = com.mardous.projectmusic.core.model.swipe.SwipeContext.ARTISTS,
            callback = this
        )
    }
    ...
    private fun setupRecyclerView() {
        ...
        headerAdapter = HeaderAdapter { view ->
            ...
            val rankInfo = buildList {
                if (artist.overallRank > 0) add("Rank: #${artist.overallRank}")
                if (artist.playCount > 0) add("Played: ${artist.playCount}x")
                if (artist.listeningDuration > 0) add("Listened: ${artist.listeningDuration.asReadableDuration(readableFormat = true)}")
            }.joinToString(" • ")

            if (Preferences.uiTheme == com.mardous.projectmusic.util.UITheme.SPOTIFY) {
                ...
                headerBinding.image.artistImage(artist) { crossfade(false) }

                headerBinding.title.text = artist.displayName()
                headerBinding.subtitle.text = artist.artistInfo(requireContext())
                headerBinding.description.text = rankInfo
                headerBinding.description.isVisible = rankInfo.isNotEmpty()

                headerBinding.playAction.setOnClickListener {
                    playerViewModel.openQueue(artist.sortedSongs, shuffleMode = OpenShuffleMode.Off, queueSource = "artist")
                }
                headerBinding.shuffleAction.setOnClickListener {
                    playerViewModel.openAndShuffleQueue(artist.sortedSongs)
                }
            } else {
    ...
    private fun artistInfo(lastFmArtist: LastFmArtist?) {
        if (lastFmArtist?.artist?.bio != null) {
            ...
            if (bioContent != null && bioContent.trim().isNotEmpty()) {
                ...
                detailViewModel.setAbout(biography)
