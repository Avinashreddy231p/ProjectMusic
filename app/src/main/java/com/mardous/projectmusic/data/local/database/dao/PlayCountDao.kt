...
interface PlayCountDao {
    ...
    @Query("DELETE FROM play_counts WHERE id = :songId")
    suspend fun deleteSongInPlayCount(songId: Long)

    @Query("DELETE FROM play_counts WHERE id IN(:songIds)")
    suspend fun deleteSongsInPlayCount(songIds: List<Long>)

    @Query("SELECT * FROM play_counts WHERE id IN (:songIds)")
    suspend fun findSongsExistInPlayCount(songIds: List<Long>): List<PlayCountEntity>

    @Query("SELECT * FROM play_counts WHERE id =:songId LIMIT 1")
    ...
    @Query("SELECT * FROM play_counts WHERE play_count > 0 ORDER BY play_count DESC LIMIT :limit")
