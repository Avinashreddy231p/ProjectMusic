...
import com.mardous.projectmusic.extensions.utilities.mapIfValid
import com.mardous.projectmusic.extensions.utilities.takeOrDefault
import com.mardous.projectmusic.util.cursor.SortedCursorUtil
...
interface PlaylistRepository {
    ...
    suspend fun insertSongs(playlistId: Long, songs: List<Song>)
    ...
    suspend fun deleteSongsFromPlaylist(playlistId: Long, songsIds: List<Long>)
...
class RealPlaylistRepository(
    ...
    override suspend fun toggleFavorite(song: Song): Boolean {
        ...
        val isFavorite = isSongFavorite(song.id)
        return if (isFavorite) {
            removeSongFromPlaylist(playlist.playListId, song.id)
            false
        } else {
            insertSongs(playlist.playListId, listOf(song))
            true
        }
    }
    ...
    override suspend fun removeSongFromPlaylist(playlistId: Long, songId: Long) =
        playlistDao.deleteSongsFromPlaylist(playlistId, listOf(songId))
    ...
    override suspend fun deleteSongsFromPlaylist(playlistId: Long, songsIds: List<Long>) {
        songsIds.chunked(MAX_ITEMS_PER_CHUNK).forEach {
            playlistDao.deleteSongsFromPlaylist(playlistId, it)
        }
    }
