...
import com.mardous.projectmusic.data.local.MetadataWriter
import com.mardous.projectmusic.data.local.lyrics.lrc.LrcLyricsParser
import com.mardous.projectmusic.data.local.lyrics.ttml.TtmlLyricsParser
...
import com.mardous.projectmusic.data.model.lyrics.LyricsFile
    ...
    override suspend fun embeddedLyrics(song: Song): RawLyrics.Embedded? {
        if (song.id != Song.emptySong.id) {
            ...
            try {
                ...
                val existingLyrics = lyricsDao.getLyrics(song.id)
                ...
                if (existingLyrics != null) {
                    lyricsDao.insertLyrics(
                        LyricsEntity(
                            ...
                            provider = existingLyrics.provider,
                            instrumental = existingLyrics.instrumental,
                            checksum = existingLyrics.checksum,
                            lastUpdated = existingLyrics.lastUpdated,
                            providerId = existingLyrics.providerId,
                            synchronizedLyrics = existingLyrics.synchronizedLyrics,
                            unsynchronizedLyrics = existingLyrics.unsynchronizedLyrics,
                            language = existingLyrics.language,
                            source = existingLyrics.source,
                            wordCount = existingLyrics.wordCount,
                            embeddedLyrics = lyrics,
                            hasEmbeddedLyrics = hasLyrics,
                            hasSynchronizedLyrics = existingLyrics.hasSynchronizedLyrics
                        )
                    )
                } else {
                    lyricsDao.insertLyrics(
