...
import com.mardous.projectmusic.core.model.Cutoff
import com.mardous.projectmusic.core.model.action.FolderAction
...
import com.mardous.projectmusic.core.model.shuffle.GroupShuffleMode
import com.mardous.projectmusic.data.model.network.NetworkFeature
import com.mardous.projectmusic.core.model.theme.AppTheme
...
import com.mardous.projectmusic.extensions.intRes
import com.mardous.projectmusic.extensions.utilities.calendarSingleton
import com.mardous.projectmusic.extensions.utilities.deserialize
import com.mardous.projectmusic.extensions.utilities.getCutoffTimeMillis
import com.mardous.projectmusic.extensions.utilities.serialize
import com.mardous.projectmusic.extensions.utilities.toEnum
import com.mardous.projectmusic.ui.component.views.TopAppBarLayout
...
object Preferences : KoinComponent {
    ...
    val blackTheme: Boolean
        get() = preferences.safeBoolean(BLACK_THEME, false)
    ...
    val isRememberLastPage: Boolean
        get() = preferences.safeBoolean(REMEMBER_LAST_PAGE, true)

    var lastPage: Int
        get() = preferences.safeInt(LAST_PAGE, 0)
    ...
    var lockedPlaylists: Boolean
        get() = preferences.safeBoolean(LOCKED_PLAYLISTS, false)
        set(value) = preferences.edit { putBoolean(LOCKED_PLAYLISTS, value) }

    val lastfmSyncFavorites: Boolean
        get() = preferences.safeBoolean(LASTFM_SYNC_FAVORITES, false)
    ...
    val lastfmOfflineScrobbling: Boolean
        get() = preferences.safeBoolean(LASTFM_OFFLINE_SCROBBLING, true)
    ...
    val lastfmScrobblePercentage: Int
        get() = preferences.safeInt(LASTFM_SCROBBLE_PERCENTAGE, 50)

    fun setLastfmScrobblePercentage(value: Int) = preferences.edit {
        putInt(LASTFM_SCROBBLE_PERCENTAGE, value)
    }

    var lastfmScrobbling: Boolean
        get() = preferences.safeBoolean(LASTFM_SCROBBLE_ENABLED, false)
        set(value) = preferences.edit { putBoolean(LASTFM_SCROBBLE_ENABLED, value) }

    var listenbrainzScrobbling: Boolean
        get() = preferences.safeBoolean(LISTENBRAINZ_SCROBBLE_ENABLED, false)
        set(value) = preferences.edit { putBoolean(LISTENBRAINZ_SCROBBLE_ENABLED, value) }

    var lastfmNowPlaying: Boolean
        get() = preferences.safeBoolean(LASTFM_NOW_PLAYING_ENABLED, false)
        set(value) = preferences.edit { putBoolean(LASTFM_NOW_PLAYING_ENABLED, value) }

    var listenbrainzNowPlaying: Boolean
        get() = preferences.safeBoolean(LISTENBRAINZ_NOW_PLAYING_ENABLED, false)
        set(value) = preferences.edit { putBoolean(LISTENBRAINZ_NOW_PLAYING_ENABLED, value) }

    var queueHeight: Boolean
        get() = preferences.safeBoolean(QUEUE_HEIGHT, false)
        set(value) = preferences.edit { putBoolean(QUEUE_HEIGHT, value) }

    val largerHeaderImage: Boolean
        get() = preferences.safeBoolean(LARGER_HEADER_IMAGE, false)

    var horizontalArtistAlbums: Boolean
        get() = preferences.safeBoolean(HORIZONTAL_ARTIST_ALBUMS, true)
        set(value) = preferences.edit { putBoolean(HORIZONTAL_ARTIST_ALBUMS, value) }

    var compactAlbumSongView: Boolean
        get() = preferences.safeBoolean(COMPACT_ALBUM_SONG_VIEW, false)
        set(value) = preferences.edit { putBoolean(COMPACT_ALBUM_SONG_VIEW, value) }

    var compactArtistSongView: Boolean
        get() = preferences.safeBoolean(COMPACT_ARTIST_SONG_VIEW, false)
        set(value) = preferences.edit { putBoolean(COMPACT_ARTIST_SONG_VIEW, value) }


    // New Era Tokens
    var eraPrimarySeed: Int
        get() = preferences.safeInt(ERA_PRIMARY_SEED, 0xFF4D5C92.toInt()) // primaryLight
        set(value) = preferences.edit { putInt(ERA_PRIMARY_SEED, value) }

    var eraSecondarySeed: Int
        get() = preferences.safeInt(ERA_SECONDARY_SEED, 0xFF595D72.toInt()) // secondaryLight
        set(value) = preferences.edit { putInt(ERA_SECONDARY_SEED, value) }

    var eraTertiarySeed: Int
        get() = preferences.safeInt(ERA_TERTIARY_SEED, 0xFF75546F.toInt()) // tertiaryLight
        set(value) = preferences.edit { putInt(ERA_TERTIARY_SEED, value) }

    var eraErrorSeed: Int
        get() = preferences.safeInt(ERA_ERROR_SEED, 0xFFBA1A1A.toInt()) // errorLight
        set(value) = preferences.edit { putInt(ERA_ERROR_SEED, value) }

    var eraHarmonyMode: Boolean
        get() = preferences.safeBoolean(ERA_HARMONY_MODE, true)
        set(value) = preferences.edit { putBoolean(ERA_HARMONY_MODE, value) }

    var eraVibrancy: Float
        get() = preferences.safeFloat(ERA_VIBRANCY, 1.0f)
        set(value) = preferences.edit { putFloat(ERA_VIBRANCY, value) }

    var eraContrast: Float
        get() = (preferences.safeInt(ERA_CONTRAST, 0) - 10) / 10.0f
        set(value) = preferences.edit { putInt(ERA_CONTRAST, (value * 10).toInt() + 10) }

    var eraShapeScale: Float
        get() = preferences.safeInt(ERA_SHAPE_SCALE, 10) / 10.0f
    ...
    var eraAsymmetricShapes: Boolean
        get() = preferences.safeBoolean(ERA_ASYMMETRIC_SHAPES, false)
        set(value) = preferences.edit { putBoolean(ERA_ASYMMETRIC_SHAPES, value) }

    var eraTypeScale: Float
        get() = preferences.safeInt(ERA_TYPE_SCALE, 10) / 10.0f
        set(value) = preferences.edit { putInt(ERA_TYPE_SCALE, (value * 10).toInt()) }

    var eraMotionIntensity: Int
        get() = preferences.requireString(ERA_MOTION_INTENSITY, "1").toIntOrNull() ?: 1 // 0: None, 1: Fluid, 2: Expressive
    ...
    var eraFontFamily: EraFont
        get() = preferences.enumValue(ERA_FONT_FAMILY, EraFont.GOOGLE_SANS)
        set(value) = preferences.edit { putString(ERA_FONT_FAMILY, value.name) }
    ...
    var vibrantBackgroundAnimations: Boolean
        get() = preferences.safeBoolean(VIBRANT_BACKGROUND_ANIMATIONS, true)
        set(value) = preferences.edit { putBoolean(VIBRANT_BACKGROUND_ANIMATIONS, value) }

    var vibrantBackgroundHighQuality: Boolean
        get() = preferences.safeBoolean(VIBRANT_BACKGROUND_HIGH_QUALITY, true)
        set(value) = preferences.edit { putBoolean(VIBRANT_BACKGROUND_HIGH_QUALITY, value) }

    var vibrantBackgroundNoiseLevel: Int
        get() = preferences.safeInt(VIBRANT_BACKGROUND_NOISE_LEVEL, 20)
        set(value) = preferences.edit { putInt(VIBRANT_BACKGROUND_NOISE_LEVEL, value) }

    var vibrantBackgroundGlobal: Boolean
        get() = preferences.safeBoolean(VIBRANT_BACKGROUND_GLOBAL, false)
        set(value) = preferences.edit { putBoolean(VIBRANT_BACKGROUND_GLOBAL, value) }

    val extraControls: Boolean
        get() = preferences.safeBoolean(ADD_EXTRA_CONTROLS, false)

    val progressBarStyle: ProgressBarStyle
        get() {
            if (preferences.safeBoolean(SQUIGGLY_SEEK_BAR, false)) {
                return ProgressBarStyle.WAVY
            }
            return preferences.enumValue(PROGRESS_BAR_STYLE, ProgressBarStyle.LINEAR)
        }

    fun setProgressBarStyle(style: ProgressBarStyle) {
        preferences.edit {
            putString(PROGRESS_BAR_STYLE, style.name)
            // Ensure squigglySeekBar is synced for legacy components
            putBoolean(SQUIGGLY_SEEK_BAR, style == ProgressBarStyle.WAVY)
        }
    }

    val thumbStyle: ThumbStyle
        get() = preferences.enumValue(THUMB_STYLE, ThumbStyle.CIRCLE)

    fun setThumbStyle(style: ThumbStyle) {
        preferences.edit { putString(THUMB_STYLE, style.name) }
    }

    var thumbSize: Float
        get() = preferences.safeFloat(THUMB_SIZE, 1.0f)
        set(value) = preferences.edit { putFloat(THUMB_SIZE, value) }

    val progressControlStyle: ProgressControlStyle
        get() = preferences.enumValue(PROGRESS_CONTROL_STYLE, ProgressControlStyle.EXPRESSIVE)

    fun setProgressControlStyle(style: ProgressControlStyle) {
        preferences.edit { putString(PROGRESS_CONTROL_STYLE, style.name) }
    }

    val adaptiveControls: Boolean
        get() = preferences.safeBoolean(ADAPTIVE_CONTROLS, false)

    val squigglySeekBar: Boolean
        get() = preferences.safeBoolean(SQUIGGLY_SEEK_BAR, false)

    val swipeDownToDismiss: Boolean
        get() = preferences.safeBoolean(SWIPE_DOWN_TO_DISMISS, false)

    var showLyricsOnCover: Boolean
        get() = preferences.safeBoolean(LYRICS_ON_COVER, false)
        set(value) = preferences.edit { putBoolean(LYRICS_ON_COVER, value) }

    val swipeOnCover: Boolean
        get() = preferences.safeBoolean(SWIPE_ON_COVER, true)

    val miniPlayerSwipeToSkip: Boolean
        get() = preferences.safeBoolean(MINI_PLAYER_SWIPE_TO_SKIP, true)

    var isQueueLocked: Boolean
        get() = preferences.safeBoolean(LOCKED_QUEUE, false)
    ...
    val isSmallImage: Boolean
        get() = preferences.safeBoolean(NOW_PLAYING_SMALL_IMAGE, false)

    fun getNowPlayingImageCornerRadius(context: Context): Int =
        preferences.safeInt(NOW_PLAYING_IMAGE_CORNER_RADIUS, context.intRes(R.integer.now_playing_corner_radius))

    fun getNowPlayingLyricsCornerRadius(context: Context): Int =
        preferences.safeInt(LYRICS_CARD_CORNER_RADIUS, 32)

    var nowPlayingLyricsCornerRadius: Int
        get() = preferences.safeInt(LYRICS_CARD_CORNER_RADIUS, 32)
        set(value) = preferences.edit { putInt(LYRICS_CARD_CORNER_RADIUS, value) }

    val isCarouselEffect: Boolean
        get() = preferences.safeBoolean(CAROUSEL_EFFECT, false)
    ...
    val animateControls: Boolean
        get() = preferences.safeBoolean(ANIMATE_PLAYER_CONTROL, true)

    val circularPlayButton: Boolean
        get() = preferences.safeBoolean(CIRCLE_PLAY_BUTTON, false)

    val enableScrollingText: Boolean
        get() = preferences.safeBoolean(ENABLE_SCROLLING_TEXT, false)

    val displayAlbumTitle
        get() = preferences.safeBoolean(DISPLAY_ALBUM_TITLE, true)

    val displayExtraInfo: Boolean
        get() = preferences.safeBoolean(DISPLAY_EXTRA_INFO, false)
    ...
    var preferRemainingTime: Boolean
        get() = preferences.safeBoolean(PREFER_REMAINING_TIME, false)
        set(value) = preferences.edit { putBoolean(PREFER_REMAINING_TIME, value) }

    var preferAlbumArtistName: Boolean
        get() = preferences.safeBoolean(PREFER_ALBUM_ARTIST_NAME, false)
    ...
    val playOptionAlwaysVisible: Boolean
        get() = preferences.safeBoolean(PLAY_OPTION_ALWAYS_VISIBLE, false)

    val playOptionClickBehavior: SongClickBehavior
        get() = if (preferences.safeBoolean(PLAY_OPTION_PLAYS_WHOLE_LIST, false)) {
    ...
    val playAllSongsWhenSearching: Boolean
        get() = preferences.safeBoolean(PLAY_ALL_SONGS_WHEN_SEARCHING, false)
    ...
    fun isResumeOnConnect(bluetooth: Boolean) = when {
        bluetooth -> preferences.safeBoolean(RESUME_ON_BLUETOOTH_CONNECT, false)
        else -> preferences.safeBoolean(RESUME_ON_CONNECT, false)
    }

    fun isPauseOnDisconnect(bluetooth: Boolean) = when {
        bluetooth -> preferences.safeBoolean(PAUSE_ON_BLUETOOTH_DISCONNECT, false)
        else -> preferences.safeBoolean(PAUSE_ON_DISCONNECT, false)
    }

    var onlyAlbumArtists: Boolean
        get() = preferences.safeBoolean(ONLY_ALBUM_ARTISTS, true)
        set(value) = preferences.edit { putBoolean(ONLY_ALBUM_ARTISTS, value) }

    val trashMusicFiles: Boolean
        get() = preferences.safeBoolean(TRASH_MUSIC_FILES, false)
    ...
    var ignoreSingles: Boolean
        get() = preferences.safeBoolean(IGNORE_SINGLES, false)
        set(value) = preferences.edit { putBoolean(IGNORE_SINGLES, value) }

    var showAlbumDuration: Boolean
        get() = preferences.safeBoolean(SHOW_TOTAL_DURATION, false)
        set(value) = preferences.edit { putBoolean(SHOW_TOTAL_DURATION, value) }

    val whitelistEnabled: Boolean
        get() = preferences.safeBoolean(WHITELIST_ENABLED, true)

    var blacklistEnabled: Boolean
        get() = preferences.safeBoolean(BLACKLIST_ENABLED, true)
        set(value) = preferences.edit { putBoolean(BLACKLIST_ENABLED, value) }

    val minimumSongCountForArtist: Int
        get() = preferences.safeInt(ARTIST_MINIMUM_SONGS, 1)

    val minimumSongCountForAlbum: Int
        get() = preferences.safeInt(ALBUM_MINIMUM_SONGS, 1)

    val minimumSongDuration: Int
        get() = preferences.safeInt(MINIMUM_SONG_DURATION, 15)

    val rotationLockEnabled: Boolean
        get() = preferences.safeBoolean(ENABLE_ROTATION_LOCK, false)
    ...
    val experimentalUpdates: Boolean
        get() = preferences.safeBoolean(EXPERIMENTAL_UPDATES, false)

    var lastUpdateSearch: Long
        get() = preferences.safeLong(LAST_UPDATE_SEARCH, -1)
        set(value) = preferences.edit { putLong(LAST_UPDATE_SEARCH, value) }

    var lastUpdateId: Long
        get() = preferences.safeLong(LAST_UPDATE_ID, -1)
    ...
    var musicbrainzEnabled: Boolean
        get() = preferences.safeBoolean(NetworkFeature.MUSICBRAINZ_ENABLED_KEY, false)
        set(value) = preferences.edit { putBoolean(NetworkFeature.MUSICBRAINZ_ENABLED_KEY, value) }

    var hierarchyFolderView: Boolean
        get() = preferences.safeBoolean(HIERARCHY_FOLDER_VIEW, false)
    ...
    var savedArtworkCopyrightNoticeShown: Boolean
        get() = preferences.safeBoolean(SAVED_ARTWORK_COPYRIGHT_NOTICE_SHOWN, false)
        set(value) = preferences.edit { putBoolean(SAVED_ARTWORK_COPYRIGHT_NOTICE_SHOWN, value) }

    var initializedBlacklist: Boolean
        get() = preferences.safeBoolean(INITIALIZED_BLACKLIST, false)
        set(value) = preferences.edit { putBoolean(INITIALIZED_BLACKLIST, value) }

    var isSwipeAnywhere: Boolean
        get() = preferences.safeBoolean(SWIPE_ANYWHERE, false)
        set(value) = preferences.edit { putBoolean(SWIPE_ANYWHERE, value) }

    var isSwipeUpQueue: Boolean
        get() = preferences.safeBoolean(SWIPE_UP_QUEUE, false)
        set(value) = preferences.edit { putBoolean(SWIPE_UP_QUEUE, value) }

    var isShowNextSong: Boolean
        get() = preferences.safeBoolean(DISPLAY_NEXT_SONG, true)
        set(value) = preferences.edit { putBoolean(DISPLAY_NEXT_SONG, value) }

    fun getSwipeLeftAction(context: com.mardous.projectmusic.core.model.swipe.SwipeContext): com.mardous.projectmusic.core.model.swipe.SwipeAction {
        val key = "${SWIPE_ACTION_LEFT_PREFIX}_${context.name.lowercase()}"
        return preferences.enumValue(key, com.mardous.projectmusic.core.model.swipe.SwipeAction.NONE)
    }

    fun getSwipeRightAction(context: com.mardous.projectmusic.core.model.swipe.SwipeContext): com.mardous.projectmusic.core.model.swipe.SwipeAction {
        val key = "${SWIPE_ACTION_RIGHT_PREFIX}_${context.name.lowercase()}"
        return preferences.enumValue(key, com.mardous.projectmusic.core.model.swipe.SwipeAction.NONE)
    }

    fun setSwipeLeftAction(context: com.mardous.projectmusic.core.model.swipe.SwipeContext, action: com.mardous.projectmusic.core.model.swipe.SwipeAction) {
        val key = "${SWIPE_ACTION_LEFT_PREFIX}_${context.name.lowercase()}"
        preferences.edit { putString(key, action.name) }
    }

    fun setSwipeRightAction(context: com.mardous.projectmusic.core.model.swipe.SwipeContext, action: com.mardous.projectmusic.core.model.swipe.SwipeAction) {
        val key = "${SWIPE_ACTION_RIGHT_PREFIX}_${context.name.lowercase()}"
        preferences.edit { putString(key, action.name) }
    }
}

fun SharedPreferences.nullString(key: String): String? = try {
    getString(key, null)
} catch (e: Exception) {
    null
}
...
fun SharedPreferences.safeInt(key: String, defaultValue: Int): Int =
    try {
        getInt(key, defaultValue)
    } catch (e: Exception) {
        getString(key, null)?.toIntOrNull() ?: defaultValue
    }

fun SharedPreferences.safeBoolean(key: String, defaultValue: Boolean): Boolean =
    try {
        getBoolean(key, defaultValue)
    } catch (e: Exception) {
        getString(key, null)?.toBoolean() ?: defaultValue
    }

fun SharedPreferences.safeFloat(key: String, defaultValue: Float): Float =
    try {
        getFloat(key, defaultValue)
    } catch (e: Exception) {
        getString(key, null)?.toFloatOrNull() ?: defaultValue
    }

fun SharedPreferences.safeLong(key: String, defaultValue: Long): Long =
    try {
...
fun SharedPreferences.safeStringSet(key: String, defaultValue: Set<String>): Set<String> =
    try {
...
inline fun <reified T : Enum<T>> SharedPreferences.enumValueByOrdinal(key: String, defaultValue: T): T =
    safeInt(key, defaultValue.ordinal).toEnum<T>() ?: defaultValue
...
enum class ProgressBarStyle {
    LINEAR, WAVY, ROUNDED, THIN, THICK, GRADIENT, SEGMENTED, DOTS, GLOW
}

enum class ThumbStyle {
    NONE, CIRCLE, PILL, DIAMOND, LINE, SQUARE, GLOW, ARROW, DOT
}

enum class ProgressControlStyle {
    CLASSIC, EXPRESSIVE
}
...
const val THUMB_STYLE = "thumb_style"
const val THUMB_SIZE = "thumb_size"
const val PROGRESS_CONTROL_STYLE = "progress_control_style"
...
const val ERA_FONT_FAMILY = "era_font_family"
