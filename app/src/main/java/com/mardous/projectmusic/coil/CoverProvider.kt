...
import com.mardous.projectmusic.coil.model.AutoGeneratedImage
...
class CoverProvider : ContentProvider(), KoinComponent {
    ...
    private val repository by lazy { GlobalContext.getOrNull()?.get<Repository>() }
    ...
    override fun openFile(uri: Uri, mode: String): ParcelFileDescriptor? {
        ...
        val result = runBlocking {
            val data = when (matchCode) {
                SONG_COVER_CODE -> repo.songById(idAsLong!!)
                ALBUM_COVER_CODE -> repo.albumById(idAsLong!!)
                ARTIST_COVER_CODE -> repo.artistById(idAsLong!!)
                ALBUM_ARTIST_COVER_CODE -> repo.albumArtistByName(id)
                PLAYLIST_COVER_CODE -> repo.playlistWithSongs(idAsLong!!)
                GENRE_COVER_CODE -> {
                    AutoGeneratedImage(
                        type = AutoGeneratedImage.TYPE_GENRE,
                        id = idAsLong!!,
                        songCount = repo.songsByGenre(idAsLong).size
                    )
                }
                else -> null
            }
